...
export class AddTutoriaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.gestionTutoriasService.info_add().subscribe((value) => {
        ...
        .subscribe(
          (value) => {
            this.router.navigate(
              ['/panel-principal/gestion-tutorias/showall'],
              {
                queryParams: {
                  flashok: this.ts.instant('gestion-tutorias.add-ok'),
                },
              }
            );
          },
            ...
            switch (error.message) {
              case '40011':
                this.error = this.ts.instant(
                  'gestion-tutorias.add-error-esp-ocupado'
                );
                break;
              case '40013':
                this.error = this.ts.instant(
                  'gestion-tutorias.add-error-pro-ocupado'
                );
                break;
              case '4002':
                this.error = this.ts.instant(
                  'gestion-tutorias.add-error-duplicado'
                );
                break;
              case '4004':
                this.error = this.ts.instant(
                  'gestion-tutorias.add-error-integridad'
                );
                break;
              default:
